...
folder_adress='http://tds.marine.rutgers.edu:8080/thredds/dodsC/met/ncep-narr/';
yearstamp='2012';
model='_narr_NENA_';
url=[folder_adress 'lwrad_down' model yearstamp '.nc']
...
time=nc{'time'}(:)+datenum(2001,1,1,0,0,0);%data are referenced to 1/1/2006

for i=1:length(ix)
    for j=1:length(iy)
        [i,j]

        url=[folder_adress 'swrad' model  yearstamp '.nc']
        nc=ncgeodataset(url);
        swrad(:,j,i)=nc{'swrad'}(:,iy(j),ix(i)); % (time, lat, lon)
        
        url=[folder_adress 'lwrad' model  yearstamp '.nc']
        nc=ncgeodataset(url);
